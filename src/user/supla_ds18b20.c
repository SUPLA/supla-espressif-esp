...
void
supla_ds18b20_write(uint8_t v, int power)
{
  uint8_t bitMask;
...
/*
char*
float2String(char* buffer, float value)
{
...
uint8_t
supla_ds18b20_crc8(uint8_t* data, uint8_t length)
{
...
supla_ds18b20_read_temperatureA(void* timer_arg)
{
  ...
  if (SUPLA_DS_MODEL == SUPLA_DS_MODEL_UNKNOWN) {
    ...
    os_timer_disarm(&supla_ds18b20_timer2);
    os_timer_setfn(&supla_ds18b20_timer2, supla_ds18b20_read_temperatureB,
                   NULL);
