...
#if defined(RGB_CONTROLLER_CHANNEL) || defined(RGBW_CONTROLLER_CHANNEL) || \
    defined(RGBWW_CONTROLLER_CHANNEL) || defined(DIMMER_CHANNEL)
...
char DEVCONN_ICACHE_FLASH supla_esp_devconn_update_started(void) {
#ifdef __FOTA
...
int DEVCONN_ICACHE_FLASH supla_esp_data_read(void *buf, int count, void *dcd) {
...
void DEVCONN_ICACHE_FLASH supla_esp_devconn_recv_cb(void *arg, char *pdata,
                                                    unsigned short len) {
  if (len == 0 || pdata == NULL) return;

  // supla_log(LOG_DEBUG, "sproto recv %i bytes, heap_size: %i", len,
...
int DEVCONN_ICACHE_FLASH supla_esp_data_write_append_buffer(void *buf,
                                                            int count) {
  if (count > 0) {
    if (devconn->esp_send_buffer_len + count > SEND_BUFFER_SIZE) {
      ...
      memcpy(&devconn->esp_send_buffer[devconn->esp_send_buffer_len], buf,
             count);
...
int DEVCONN_ICACHE_FLASH supla_esp_data_write(void *buf, int count, void *dcd) {
  int r;

  if (devconn->esp_send_buffer_len > 0) {
    if ((r = supla_espconn_sent(&devconn->ESPConn,
                                (unsigned char *)devconn->esp_send_buffer,
  ...
  if (count > 0) {
    ...
    if (ESPCONN_INPROGRESS == r) {
      ...
      if (r == 0) devconn->last_sent = uptime_sec();
...
void DEVCONN_ICACHE_FLASH supla_esp_set_state(int __pri, const char *message) {
  if (message == NULL) return;

  supla_log(__pri, message);

  char laststate[STATE_MAXSIZE];
  ets_snprintf(laststate, STATE_MAXSIZE, "%s%s%s", message,
               strnlen(devconn->laststate, STATE_MAXSIZE) > 0 ? "," : "",
               devconn->laststate);

  ets_snprintf(devconn->laststate, STATE_MAXSIZE, "%s", laststate);
}
    ...
    for (a = 0; a < RS_MAX_COUNT; a++) {
      if (supla_rs_cfg[a].up != NULL && supla_rs_cfg[a].down != NULL &&
        ...
        supla_esp_channel_value_changed(
            supla_rs_cfg[a].up->channel,
            ((*supla_rs_cfg[a].position) - 100) / 100);
      }
    }
...
supla_esp_devconn_send_channel_values_with__delay(int time_ms) {
  os_timer_disarm(&devconn->supla_value_timer);
  os_timer_setfn(&devconn->supla_value_timer,
                 (os_timer_func_t *)supla_esp_devconn_send_channel_values_cb,
                 NULL);
...
void DEVCONN_ICACHE_FLASH supla_esp_channel_set_activity_timeout_result(
...
char DEVCONN_ICACHE_FLASH supla_esp_devconn_is_registered(void) {
  return devconn != NULL && devconn->srpc != NULL && devconn->registered == 1
             ? 1
             : 0;
}

void DEVCONN_ICACHE_FLASH supla_esp_channel_value__changed(
...
void DEVCONN_ICACHE_FLASH supla_esp_channel_value_changed(int channel_number,
                                                          char v) {
  if (supla_esp_devconn_is_registered() == 1) {
    // supla_log(LOG_DEBUG, "supla_esp_channel_value_changed(%i, %i)",
    // channel_number, v);
...
void DEVCONN_ICACHE_FLASH supla_esp_channel_extendedvalue_changed(
    unsigned char channel_number, TSuplaChannelExtendedValue *value) {
  if (supla_esp_devconn_is_registered() == 1) {
    ...
    srpc_ds_async_channel_extendedvalue_changed(devconn->srpc, channel_number,
                                                value);
  }
}

#if defined(RGBW_CONTROLLER_CHANNEL) || defined(RGBWW_CONTROLLER_CHANNEL) || \
    defined(RGB_CONTROLLER_CHANNEL) || defined(DIMMER_CHANNEL)

void DEVCONN_ICACHE_FLASH
supla_esp_channel_rgbw_to_value(char value[SUPLA_CHANNELVALUE_SIZE], int color,
...
supla_esp_channel_value_changed_delayed_cb(void *timer_arg) {
  if (supla_esp_devconn_is_registered()) {
    srpc_ds_async_channel_value_changed(
        devconn->srpc, ((channel_value_delayed *)timer_arg)->channel_number,
        ((channel_value_delayed *)timer_arg)->value);
  }
}

#ifdef CVD_MAX_COUNT
void DEVCONN_ICACHE_FLASH supla_esp_channel_rgbw_value_changed(
    int channel_number, int color, char color_brightness, char brightness) {
  if (channel_number >= CVD_MAX_COUNT) return;

  devconn->cvd[channel_number].channel_number = channel_number;

  supla_esp_channel_rgbw_to_value(devconn->cvd[channel_number].value, color,
                                  color_brightness, brightness);

  os_timer_disarm(&devconn->cvd[channel_number].timer);
  os_timer_setfn(&devconn->cvd[channel_number].timer,
                 (os_timer_func_t *)supla_esp_channel_value_changed_delayed_cb,
                 &devconn->cvd[channel_number]);
...
char DEVCONN_ICACHE_FLASH _supla_esp_channel_set_value(int port, char v,
                                                       int channel_number) {
...
void DEVCONN_ICACHE_FLASH
supla_esp_devconn_on_countdown_on_disarm(uint8 channel_number) {
...
void DEVCONN_ICACHE_FLASH supla_esp_devconn_on_countdown_timer_finish(
    uint8 gpio_id, uint8 channel_number,
    char target_value[SUPLA_CHANNELVALUE_SIZE]) {
  _supla_esp_channel_set_value(
...
#ifndef SUPLA_SMOOTH_DISABLED
void DEVCONN_ICACHE_FLASH supla_esp_devconn_smooth_brightness(
    float *brightness, float *dest_brightness, float *step) {
  if (*brightness > *dest_brightness) {
    *brightness = *brightness - *step;

    if (*brightness < 0) *brightness = 0;

    if (*brightness < *dest_brightness) *brightness = *dest_brightness;
...
void DEVCONN_ICACHE_FLASH supla_esp_devconn_smooth_cb(devconn_smooth *_smooth) {
  ...
  supla_esp_devconn_smooth_brightness(&_smooth->color_brightness,
                                      &_smooth->dest_color_brightness,
                                      &_smooth->color_brightness_step);
  supla_esp_devconn_smooth_brightness(&_smooth->brightness,
                                      &_smooth->dest_brightness,
                                      &_smooth->brightness_step);
  ...
  supla_esp_board_set_rgbw_value(_smooth->ChannelNumber, &_smooth->color,
                                 &_smooth->color_brightness,
                                 &_smooth->brightness, _smooth->turn_onoff);
#else
  supla_esp_board_set_rgbw_value(_smooth->ChannelNumber, &_smooth->color,
                                 &_smooth->color_brightness,
                                 &_smooth->brightness);
...
void _supla_esp_devconn_smooth_cb(void) {
  int a;
  char job = 0;
...
#ifdef RGBW_ONOFF_SUPPORT
void DEVCONN_ICACHE_FLASH supla_esp_channel_set_rgbw_value(
    int ChannelNumber, int Color, char ColorBrightness, char Brightness,
    char TurnOnOff, char smoothly, char send_value_changed) {
#else
void DEVCONN_ICACHE_FLASH supla_esp_channel_set_rgbw_value(
    int ChannelNumber, int Color, char ColorBrightness, char Brightness,
  ...
  supla_esp_board_get_rgbw_value(ChannelNumber, &_smooth->color,
                                 &_smooth->color_brightness,
                                 &_smooth->brightness);

  _smooth->color_brightness_step =
      abs(_smooth->color_brightness - ColorBrightness) / 100.00;
...
supla_esp_channel_set_value(TSD_SuplaChannelNewValue *new_value) {
  ...
  if (new_value->ChannelNumber == rgb_cn ||
    ...
    supla_esp_channel_set_rgbw_value(new_value->ChannelNumber, Color,
                                     ColorBrightness, Brightness, TurnOnOff, 1,
                                     1);
#else
    supla_esp_channel_set_rgbw_value(new_value->ChannelNumber, Color,
  ...
  /*
      char buff[200];
      ets_snprintf(buff, 200, "set_value %i,%i,%i", new_value->value[0],
     new_value->ChannelNumber, new_value->SenderID); supla_esp_write_log(buff);
  */
  ...
  if (new_value->DurationMS > 0) {
    for (a = 0; a < RELAY_MAX_COUNT; a++)
      if (supla_relay_cfg[a].gpio_id != 255 &&
          new_value->ChannelNumber == supla_relay_cfg[a].channel) {
        if (v == 1 || supla_relay_cfg[a].channel_flags &
                          SUPLA_CHANNEL_FLAG_COUNTDOWN_TIMER_SUPPORTED) {
          char target_value[SUPLA_CHANNELVALUE_SIZE];
          memcpy(target_value, new_value->value, SUPLA_CHANNELVALUE_SIZE);
          target_value[0] = v ? 0 : 1;

          supla_esp_countdown_timer_countdown(
              new_value->DurationMS, supla_relay_cfg[a].gpio_id,
              supla_relay_cfg[a].channel, target_value, new_value->SenderID);
          ...
          if (supla_relay_cfg[a].channel_flags &
            ...
            if (ev != NULL) {
              ...
              supla_esp_channel_extendedvalue_changed(new_value->ChannelNumber,
                                                      ev);
              free(ev);
            }
          }
...
void supla_esp_devconn_iterate(void *timer_arg) {
  if (devconn->srpc != NULL) {
    if (devconn->registered == 0) {
      ...
      if (strlen(supla_esp_cfg.Email) > 0) {
        ...
        ets_snprintf(srd.ServerName, SUPLA_SERVER_NAME_MAXSIZE, "%s",
        ...
        ets_snprintf(srd.LocationPWD, SUPLA_LOCATION_PWD_MAXSIZE, "%s",
                     supla_esp_cfg.LocationPwd);
        ets_snprintf(srd.ServerName, SUPLA_SERVER_NAME_MAXSIZE, "%s",
                     supla_esp_cfg.Server);
...
void DEVCONN_ICACHE_FLASH supla_esp_srpc_init(void) {
  ...
  os_timer_setfn(&devconn->supla_iterate_timer,
                 (os_timer_func_t *)supla_esp_devconn_iterate, NULL);
...
void DEVCONN_ICACHE_FLASH supla_espconn_disconnect(struct espconn *espconn) {
  ...
  if (espconn->state != ESPCONN_CLOSE && espconn->state != ESPCONN_NONE) {
    _supla_espconn_disconnect(espconn);
  }
}

void DEVCONN_ICACHE_FLASH supla_esp_devconn_connect_cb(void *arg) {
...
void DEVCONN_ICACHE_FLASH supla_esp_devconn_init(void) {
  ...
  os_timer_disarm(&devconn->supla_watchdog_timer);
  os_timer_setfn(&devconn->supla_watchdog_timer,
                 (os_timer_func_t *)supla_esp_devconn_watchdog_cb, NULL);
  os_timer_arm(&devconn->supla_watchdog_timer, 1000, 1);

  supla_esp_countdown_set_on_disarm_cb(
      supla_esp_devconn_on_countdown_on_disarm);
  supla_esp_countdown_set_finish_cb(
...
void DEVCONN_ICACHE_FLASH supla_esp_devconn_timer1_cb(void *timer_arg) {
  ...
  if (supla_esp_devconn_is_registered() == 1 &&
      devconn->server_activity_timeout > 0 && devconn->srpc != NULL) {
...
#ifdef ELECTRICITY_METER_COUNT
void DEVCONN_ICACHE_FLASH supla_esp_channel_em_value_changed(
    unsigned char channel_number, TElectricityMeter_ExtendedValue_V2 *em_ev) {
  TSuplaChannelExtendedValue *ev =
