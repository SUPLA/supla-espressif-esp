...
void ICACHE_FLASH_ATTR supla_esp_board_gpio_init(void)
{
    ...
    supla_input_cfg[1].type = supla_esp_cfg.Button2Type == BTN_TYPE_BISTABLE
        ? INPUT_TYPE_BTN_BISTABLE
        : INPUT_TYPE_BTN_MONOSTABLE;
...
*/
...
#if defined(__BOARD_inCan_DS)
    supla_input_cfg[6].type = INPUT_TYPE_TEMPERATURE_DS18B20;
    ...
    supla_input_cfg[6].type = INPUT_TYPE_TEMPERATURE_HUMIDITY_DHT22;
    supla_input_cfg[6].gpio_id = B_TEMPERATURE;
    supla_input_cfg[6].channel = 1;
#endif
    ...
    char dev_name[25], const char mac[6], const char data_saved)
{
    ...
    static char html_template[] = "%s%s<div class=\"s\"><svg version=\"1.1\" id=\"l\" x=\"0\" y=\"0\" "
                                  ...
                                  "<i><select name=\"bt2\"><option value=\"0\" %s>Monostable<option "
                                  "value=\"1\" %s>Bistable</select><label>Button2 type:</label></i>"
#endif
...
// supla_log(LOG_DEBUG, "RELAY HI");
        ...
        if (rs_cfg != NULL) {
            supla_esp_gpio_rs_set_relay(
                rs_cfg,
                rs_cfg->up->gpio_id == input_cfg->relay_gpio_id ? RS_RELAY_UP
                                                                : RS_RELAY_DOWN,
                1, 1);
        }
