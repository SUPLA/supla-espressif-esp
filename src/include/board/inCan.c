...
void supla_esp_board_set_device_name(char *buffer, uint8 buffer_size) {
	...
	ets_snprintf(buffer, buffer_size, "inCan-DS18b20");

#elif defined(__BOARD_inCan_DHT11)

    ets_snprintf(buffer, buffer_size, "inCan-DHT11");

#elif defined(__BOARD_inCan_DHT22)

    ets_snprintf(buffer, buffer_size, "inCan-DHT22");

#elif defined(__BOARD_inCanRS_DS)

	ets_snprintf(buffer, buffer_size, "inCanRS-DS18b20");

#elif defined(__BOARD_inCanRS_DHT11)

    ets_snprintf(buffer, buffer_size, "inCanRS-DHT11");

#elif defined(__BOARD_inCanRS_DHT22)

    ets_snprintf(buffer, buffer_size, "inCanRS-DHT22");
...
void ICACHE_FLASH_ATTR supla_esp_board_gpio_init(void) {
	...
	#if defined(__BOARD_inCan_DS) || defined(__BOARD_inCan_DHT11) || defined(__BOARD_inCan_DHT22)

        supla_input_cfg[0].type = supla_esp_cfg.Button1Type == BTN_TYPE_BISTABLE ? INPUT_TYPE_BTN_BISTABLE : INPUT_TYPE_BTN_MONOSTABLE;
        supla_input_cfg[0].gpio_id = B_BTN1_PORT;
        supla_input_cfg[0].flags = INPUT_FLAG_PULLUP;
        supla_input_cfg[0].relay_gpio_id = B_RELAY2_PORT;
        supla_input_cfg[0].channel = 2;

        supla_input_cfg[1].type = supla_esp_cfg.Button2Type == BTN_TYPE_BISTABLE ? INPUT_TYPE_BTN_BISTABLE : INPUT_TYPE_BTN_MONOSTABLE;
        ...
        supla_input_cfg[2].gpio_id = B_CFG_PORT;
        supla_input_cfg[2].flags = INPUT_FLAG_PULLUP | INPUT_FLAG_CFG_BTN;

        supla_input_cfg[3].type = INPUT_TYPE_SENSOR;
        supla_input_cfg[3].gpio_id = B_SENSOR1_PORT;
        supla_input_cfg[3].channel = 0;
/*
        supla_input_cfg[4].type = INPUT_TYPE_SENSOR;
        supla_input_cfg[4].gpio_id = B_SENSOR2_PORT;
        supla_input_cfg[4].channel = 1;
*/
    ...
    #elif defined(__BOARD_inCanRS_DS) || defined(__BOARD_inCanRS_DHT11) || defined(__BOARD_inCanRS_DHT22)
        ...
        supla_input_cfg[4].type = INPUT_TYPE_SENSOR;
        supla_input_cfg[4].gpio_id = B_WEJSCIE;
...
void ICACHE_FLASH_ATTR supla_esp_board_set_channels(TDS_SuplaDeviceChannel_B *channels, unsigned char *channel_count) {
		...
		*channel_count = 3;
		...
		channels[1].Type = SUPLA_CHANNELTYPE_DHT11;
		...
		if(supla_input_cfg[0].type == INPUT_TYPE_BTN_MONOSTABLE){
			channels[2].FuncList = SUPLA_BIT_RELAYFUNC_LIGHTSWITCH \
		...
		if(supla_input_cfg[1].type == INPUT_TYPE_BTN_MONOSTABLE){
			channels[3].FuncList = SUPLA_BIT_RELAYFUNC_LIGHTSWITCH \
											| SUPLA_BIT_RELAYFUNC_POWERSWITCH;
		}
		else {
			channels[3].FuncList = SUPLA_BIT_RELAYFUNC_LIGHTSWITCH \
	...
	#elif defined(__BOARD_inCanRS_DS) || defined(__BOARD_inCanRS_DHT11) || defined(__BOARDRS_inCanRS_DHT22)
...
void ICACHE_FLASH_ATTR supla_esp_board_send_channel_values_with_delay(void *srpc) {
	...
	#if defined(__BOARD_inCan_DS) || defined(__BOARD_inCan_DHT11) || defined(__BOARD_inCan_DHT22)
		supla_esp_channel_value_changed(2, supla_esp_gpio_relay_on(B_RELAY1_PORT));
...
void ICACHE_FLASH_ATTR supla_esp_board_gpio_relay_switch(void *_input_cfg, char hi) {
