...
TEST_F(RollerShutterAutoCalWithSrpc, RsAutoCalibrated_SetNewPosition) {
  ...
  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({19, 0, 0, 0, 0, 0, 0, 0})))
      .WillOnce(Return(0));
  ...
  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({38, 0, 0, 0, 0, 0, 0, 0})))
      .WillOnce(Return(0));

  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({58, 0, 0, 0, 0, 0, 0, 0})))
      .WillOnce(Return(0));

  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({78, 0, 0, 0, 0, 0, 0, 0})))
      .WillOnce(Return(0));
...

  {
    ...
    EXPECT_CALL(
        srpc, valueChanged(_, 0,
          ElementsAreArray(
            {0xFF, 0, 0, RS_VALUE_FLAG_CALIBRATION_IN_PROGRESS,
            0, 0, 0, 0})))
      .WillOnce(Return(0));
    ...
    // after calibration we set position 80 (intermediate step values are
    // ignored)
    EXPECT_CALL(srpc, valueChanged(_, 0, _)).Times(6);
  }
...
TEST_F(RollerShutterAutoCalF, RsNotCalibrated_TriggerAutoCalAndAbort) {
  // add task may be executed i.e. in mqtt
...
TEST_F(RollerShutterAutoCalF, AutoCalibrationFromCfgmodeOrMqtt) {
  ...
  supla_esp_gpio_rs_apply_new_times(0, 2000, 2200, 0);
  ...
  supla_esp_gpio_rs_apply_new_times(0, 0, 0, 0);
  supla_esp_gpio_rs_add_task(0, 20, 0);
...
TEST_F(RollerShutterAutoCalF, SwitchToAutoCalibrationDuringManualCalibration) {
  ...
  // manual calibration times
  supla_esp_gpio_rs_apply_new_times(0, 2000, 2000, 0);
  supla_esp_gpio_rs_add_task(0, 20, 0);
...
TEST_F(RollerShutterAutoCalWithSrpc,
       AutoCalibrationFailedTooShortTimeOnMoveUp) {
  int curTime = 10000;  // start at +10 ms
  EXPECT_CALL(time, system_get_time()).WillRepeatedly(ReturnPointee(&curTime));
  ...
  // Second calibration attempt should be successful and should clear error
  // flags Set how long [ms] "is_in_move" method will return true auto cal times
  // < 500 ms are considered as errors
  upTime = 1100;
  downTime = 1200;
...
TEST_F(RollerShutterAutoCalWithSrpc, RsAutoCalibrated_CalibrationLost) {
  ...
  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({39, 0, 0, 0, 0, 0, 0, 0})))
      .WillOnce(Return(0));

  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({59, 0, 0, 0, 0, 0, 0, 0})))
      .WillOnce(Return(0));

  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({79, 0, 0, 0, 0, 0, 0, 0})))
      .WillOnce(Return(0));
  ...
  EXPECT_CALL(srpc, valueChanged(_, 0,
                                 ElementsAreArray(
                                     {100, 0, 0, RS_VALUE_FLAG_CALIBRATION_LOST,
                                      0, 0, 0, 0})))
      .WillOnce(Return(0));
...
TEST_F(RollerShutterAutoCalWithSrpc, RsAutoCalibrated_CalibrationLostPos0) {
  ...
  EXPECT_CALL(
      ...
      valueChanged(_, 0,
                   ElementsAreArray(
                       {0, 0, 0, RS_VALUE_FLAG_CALIBRATION_LOST, 0, 0, 0, 0})))
      .WillOnce(Return(0));
...
TEST_F(RollerShutterAutoCalWithSrpc, RsAutoCalibrated_CalibrationLostMoveDown) {
  ...
  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({40, 0, 0, 0, 0, 0, 0, 0})))
      .WillRepeatedly(Return(0));

  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({60, 0, 0, 0, 0, 0, 0, 0})))
      .WillRepeatedly(Return(0));

  EXPECT_CALL(srpc,
              valueChanged(_, 0, ElementsAreArray({80, 0, 0, 0, 0, 0, 0, 0})))
      .WillRepeatedly(Return(0));
  ...
  EXPECT_CALL(
      srpc,
      valueChanged(_, 0,
                   ElementsAreArray({100, 0, 0, RS_VALUE_FLAG_CALIBRATION_LOST,
                                     0, 0, 0, 0})))
      .WillOnce(Return(0));
